...
eval (App (Lambda var funExpr) arg) c m = eval (App lambda arg) c2 m2
    where
        (val, c2, m2) = eval (head arg) c m
        (lambda, _, _) = eval (Lambda var funExpr) c m

eval (App (Function var funExpr context) [(Name arg)]) c m = (res, c2, m2)
    where
        context2 = addToCtx context var (fromJust (lookupCtx c arg))
        (res, retCtx, retMem) = eval funExpr context2 m
        (i2, m2) = addToMem m (fromJust (lookupMem retMem (fromJust (lookupCtx retCtx var))))
        c2 = addToCtx c arg i2

eval (App (Function var funExpr context) arg) c m = (res, c2, m4)
    where
        (val, c2, m2) = eval (head arg) c m
